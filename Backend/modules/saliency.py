...
def download_weights(model_dir="models"):
    url = "https://huggingface.co/netradrishti/u2net-saliency/resolve/main/models/u2netp.pth"
    os.makedirs(model_dir, exist_ok=True)
    weight_path = os.path.join(model_dir, "u2netp.pth")
    if not os.path.exists(weight_path):
        print(f"Downloading U2NetP weights to {weight_path}...")
...
def get_saliency_map(image_path, weight_path):
    ...
    sal_img = Image.fromarray((predict_np * 255).astype(np.uint8)).resize(orig_size, resample=Image.BILINEAR)
